...
const _EmailRequired = (message = 'Bu alan gereklidir.') => v.pipe(base.email, v.nonEmpty(message), v.metadata({ slc_required: true }));
...
export function Email<Key extends string, Required extends boolean = true>(key: Key, options: { required?: Required; message?: string } = {}) {
