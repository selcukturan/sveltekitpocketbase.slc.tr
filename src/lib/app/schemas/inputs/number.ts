...
const _NumberRequiredPositiveInteger = ({ message = 'Bu alan gereklidir. Pozitif tamsayı olmalıdır.' }: { message?: string } = {}) => {
	return v.pipe(
		base.integer,
...
const _NumberRequiredNegativeInteger = ({ message = 'Bu alan gereklidir. Negatif tamsayı olmalıdır.' }: { message?: string } = {}) => {
	return v.pipe(
		base.integer,
...
const _NumberRequiredPositiveDecimal = ({ message = 'Bu alan gereklidir. 0 dan büyük olmalıdır.', step = 2 }: { message?: string; step?: number } = {}) => {
	const stepVal = step <= 0 ? 2 : step;
...
const _NumberRequiredNegativeDecimal = ({ message = 'Bu alan gereklidir. 0 dan küçük olmalıdır.', step = 2 }: { message?: string; step?: number } = {}) => {
	const stepVal = step <= 0 ? 2 : step;
...
const _NumberOptionalPositiveDecimal = ({ step = 2 }: { step?: number } = {}) => {
	const stepVal = step <= 0 ? 2 : step;
	return v.pipe(
		v.optional(v.pipe(base.number, base.maxDecimalPlaces(stepVal), v.minValue(0, 'Sayısal değer 0 veya daha büyük olmalıdır.')), 0),
		v.metadata({
			slc_required: false
		})
	);
...
const _NumberOptionalNegativeDecimal = ({ step = 2 }: { step?: number } = {}) => {
	const stepVal = step <= 0 ? 2 : step;
	return v.pipe(
		v.optional(v.pipe(base.number, base.maxDecimalPlaces(stepVal), v.maxValue(0, 'Sayısal değer 0 veya daha küçük olmalıdır.')), 0),
		v.metadata({
			slc_required: false
		})
	);
...
type NumberTypeChoice<Type extends 'integer' | 'decimal', Sign extends 'positive' | 'negative' | 'both', Required extends boolean> = Type extends 'integer'
	? // Integer types
