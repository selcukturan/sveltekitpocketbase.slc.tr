<script lang="ts" generics="Tmultiple extends boolean = false">
	...
	import type { RelationPickerPropsType, ValueType, ResolveData } from './types';
	...
	}: RelationPickerPropsType<Tmultiple> = $props();
	...
	let testValueMultiple = $state([
		{ value: 'html', checked: false },
		{ value: 'css', checked: false },
		{ value: 'js', checked: false }
	]);

	let testValueSingle = $state([
		{ value: 'windows', checked: false },
		{ value: 'mac', checked: false },
		{ value: 'linux', checked: false }
	]);

	let returnTestValue = $derived.by(() => {
		if (multiple) {
			return testValueMultiple.filter((item) => item.checked).map((item) => item.value) as ValueType<Tmultiple>;
		}
		return testValueSingle.find((item) => item.checked)?.value as ValueType<Tmultiple>;
	});
	/**
	...
	const show = (): Promise<ResolveData<Tmultiple>> => {
		return new Promise<ResolveData<Tmultiple>>((resolve) => {
			// TypeScript burada resolve'un tipinin (data: ResolveData<Tmultiple>) => void
			// olduğunu bilir ve resolvePromise'e atanmasına izin verir.
...
>
	<div class="dialog-content">
		...
		{#if multiple}
			<label>
				<input type="checkbox" bind:checked={testValueMultiple[0].checked} value={testValueMultiple[0].value} />
				{testValueMultiple[0].value}
			</label>
			<label>
				<input type="checkbox" bind:checked={testValueMultiple[1].checked} value={testValueMultiple[1].value} />
				{testValueMultiple[1].value}
			</label>
			<label>
				<input type="checkbox" bind:checked={testValueMultiple[2].checked} value={testValueMultiple[2].value} />
				{testValueMultiple[2].value}
			</label>
		{:else}
			<label>
				<input name="scoops" type="radio" bind:group={testValueSingle[0].checked} value={testValueSingle[0].value} />
				{testValueSingle[0].value}
			</label>
			<label>
				<input name="scoops" type="radio" bind:group={testValueSingle[1].checked} value={testValueSingle[1].value} />
				{testValueSingle[1].value}
			</label>
			<label>
				<input name="scoops" type="radio" bind:group={testValueSingle[2].checked} value={testValueSingle[2].value} />
				{testValueSingle[2].value}
			</label>
		{/if}
