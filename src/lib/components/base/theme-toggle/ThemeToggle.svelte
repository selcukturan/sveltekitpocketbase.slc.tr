...
<button
	...
	class="bg-surface-50 hover:border-surface-400 relative block h-[22px] w-[40px] rounded-xl border hover:cursor-pointer"
>
	<span
		class={`bg-surface-300 pointer-events-none absolute top-[1px] left-[1px] h-[18px] w-[18px] rounded-full shadow transition-transform ${theme.current === 'dark' ? 'translate-x-[18px]' : ''}`}
	>
		<span class="relative block h-[18px] w-[18px] overflow-hidden rounded-full [&_svg]:absolute [&_svg]:top-[3px] [&_svg]:left-[3px] [&_svg]:h-[12px] [&_svg]:w-[12px]">
			{#if browser}
				{#if theme.current === 'dark'}
