<script lang="ts">
	...
	import { formatZodError, type FieldErrors } from '$lib/client/formError'; // Hata formatlayıcıyı import et
	...
	let generalServerErrors = $derived(serverErrors?.['_general'] ?? []);
...
<Page>
	...
	<Main>
			...
			use:enhance={({ formData, cancel }) => {
				...
				try {
					...
					if (error instanceof ZodError) {
						...
						clientErrors = { ['_general']: ['Beklenmedik bir hata oluştu.'] };
					}
				...
				return async ({ result, update }) => {
					...
					if (result.type === 'redirect') {
						...
						clientErrors = { ['_general']: ['Sunucu hatası: ' + (result.error?.message ?? 'Bilinmeyen hata')] };
					}
		...
		>
			...
			{#if clientErrors['_general']}
				<p class="error">{clientErrors['_general']?.[0]}</p>
			{/if}
